...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  with tf.compat.v1.name_scope('cross_entropy'):
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
      logits=logits, labels=tf.stop_gradient(ground_truth_input))
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  with tf.compat.v1.name_scope('accuracy'):
    with tf.compat.v1.name_scope('correct_prediction'):
      correct_prediction = tf.equal(tf.argmax(input=result_tensor, axis=1), \
        tf.argmax(input=ground_truth_tensor, axis=1))
    with tf.compat.v1.name_scope('accuracy'):
